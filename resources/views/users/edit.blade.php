...
@section('breadcrumb')
    <div class="breadcrumb">
        ...
        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
        <span class="text-slate-800 dark:text-slate-200 font-medium">Edit Pengguna</span>
    </div>
@endsection
        ...
        <form method="POST" action="{{ route('users.update', $user) }}" id="form-edit-user">
            ...
            <div class="space-y-5">
                ...
                <div class="flex items-center gap-4 pb-4 border-b border-slate-100 dark:border-slate-800">
                    <img src="{{ $user->avatar_url }}" alt="{{ $user->name }}" class="avatar w-14 h-14">
                    <div>
                        <p class="text-sm font-semibold text-slate-900 dark:text-white">{{ $user->name }}</p>
                        <p class="text-xs text-slate-500">{{ $user->email }}</p>
                        @if(!$user->is_active)
                            <span class="badge badge-red text-[10px] mt-1">Nonaktif</span>
                        @endif
                    </div>
                </div>
                ...
                <div class="border-t border-slate-100 dark:border-slate-800 pt-4">
                    <p class="text-xs text-slate-500 mb-4">Kosongkan password jika tidak ingin mengubah</p>
